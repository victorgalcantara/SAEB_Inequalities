...
# ----- Merge results+socioeconomic
data_07 <- merge(TS_ALUNO_9EF,TS_QUEST_ALUNO_9EF)

# ----- Indicators

# Parameters for itens discrimination
load("discr_itens.RDS")

# Indicator of Socioeconomic Capital (SC/CE)
data_07 <- data_07 %>% 
  replace(.,. == "." | . == "*",NA)
...
save(data_07, file="data_07.RDS") # only students infos

load("estruturaEscola_2007.RDS")
data_07 <- merge(data_07,escolas_07) 

save(data_07,file="data_07_compl.RDS") # students and schools infos
