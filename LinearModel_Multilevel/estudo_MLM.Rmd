...
library(tidyverse) 
library(readxl)    
library(sjPlot)    # for print results
...
save(data,file = "mlm_data.RDS")

load("C:/Users/VictorGabriel/Documents/00_dados/mlm_data.RDS")
...
Se 100 alunos de uma escola forem avaliados, é provável que se observe uma variação grande nos resultados _dentro_ ( _within schools_ ) da escola.

Porém, é possível computar a média dos resultados em cada escola e compará-las com a média geral (todos os alunos sem distinção de escola). Esta é uma comparação _entre_ ( _between schools_ ) escolas.

Para diferenciar a variabilidade explicada pelos indivíduos e a explicada pelos agrupamentos (clusters), usamos os modelos multiníveis.
...
d <- data %>% select(ID_ESCOLA,PROFICIENCIA_LP_SAEB,CE) %>% filter(n > 2) %>% na.exclude()
...
clusters <- merge(coef,clusters_data)
...
o_hat_LP = sum( ( (n - 1)*S_LP),na.rm = T ) / (N - C)
o_hat_MT = sum( ( (n - 1)*S_MT),na.rm = T ) / (N - C)

# variables
n_tiu = (1 / (C-1) ) * ( N - (sum(n^2)/N) )

# Total variance
S_b_LP = sum ( n * ( (me_LP - me_tot_LP)^2 ) ) / (n_tiu*(C-1)) 
S_b_MT = sum ( n * ( (me_MT - me_tot_MT)^2 ) ) / (n_tiu*(C-1))

# variation between schools (clusters)
t_hat_LP = S_b_LP - (o_hat/n_tiu)
t_hat_MT = S_b_MT - (o_hat/n_tiu)

# measure of p
p_LP = t_hat_LP / (t_hat_LP + o_hat_LP)
p_MT = t_hat_MT / (t_hat_MT + o_hat_MT)
...
Com o a estatística p, temos que aproximadamente 17% da variação do desempenho em LP e 19% em MT é explicada pelas escolas. Esta é uma proporção alta da variância. Porém, devemos avançar na construção do modelo controlando por fatores intervenientes.
...
t_hat_0 = 383.85   # variance between clusters
o_hat_0 = 1947.77  # variance within clusters
...
model1.1 <- lmer(data=data,PROFICIENCIA_LP_SAEB ~ CE + educMae + educPai + sexo + raca + trabFora + regiao + zona + (1|ID_ESCOLA))
tab_model(model1.1)

t_hat_1 = 228.81   # variance between school
o_hat_1 = 1800.36  # variance within school

p = t_hat_1 / (t_hat_1 + o_hat_1)
...
model1.2 <- lmer(data=data,PROFICIENCIA_LP_SAEB ~ CE + educPais + sexo + raca + trabFora + regiao + zona + (1 + estruturaEscola + PC_FORMACAO_DOCENTE |ID_ESCOLA))
tab_model(model1.2)

t_hat_2 = 1099.79   # variance between school
o_hat_2 = 1776.52   # variance within school
