...
data_19_2 <- data_19_2 %>% mutate(.,
                              raca3=case_when(
                                raca2 == "Brancos"     ~ 1,
                                raca2 == "Não brancos" ~ 0
                              ))

escolas <- unique(data_19_2$ID_ESCOLA)
model_LP <- PROFICIENCIA_LP_SAEB ~ educMae + CE + raca2
model_MT <- PROFICIENCIA_MT_SAEB ~ educMae + CE + raca2

escola_i <- data_19_2 %>% filter(ID_ESCOLA == escolas[1])
...
for(i in 1:length(escolas)){
  escola_i <- data_19_2 %>% filter(ID_ESCOLA == escolas[i])
